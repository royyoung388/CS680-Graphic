...
class GLProgram:
    ...
    def __init__(self) -> None:
        ...
        for i in range(int(self.attribs["maxLightsNum"])):
            ...
            self.attribs[f"light[{i}].spotAngleFactor"] = f"{self.attribs['light']}[{i}].spotAngleFactor"
    ...
    def genVertexShaderSource(self):
        ...
        in vec3 {self.attribs["vertexPu"]};
        in vec3 {self.attribs["vertexPv"]};
        ...
        {{
            ...
            vPu = normalize(transpose(inverse({self.attribs["modelMat"]})) * vec4({self.attribs["vertexPu"]}, 0.0) ).xyz;
            vPv = normalize(transpose(inverse({self.attribs["modelMat"]})) * vec4({self.attribs["vertexPv"]}, 0.0) ).xyz;
        }}
        '''
        return vss
    ...
    def genFragShaderSource(self):
        ...
        {{
            ...
            // Reserved for normal mapping, routing name is "nmap"
            if ((renderingFlag >> 7 & 0x1) == 1) {{
                vec4 tColor = texture({self.attribs["normalImage"]}, vTexture);
                N = tColor.r * vPu + tColor.g * vPv + tColor.b * N;
            }}
            ...
            if ((renderingFlag >> 0 & 0x1) == 1) {{
                ...
                vec4 Ia = vec4(0.0), Id = vec4(0.0), Is = vec4(0.0);
                
                for (int i = 0; i < MAX_LIGHT_NUM; i++) {{
                    Light l = {self.attribs["light"]}[i];
                    
                    // skip off light
                    if (!l.lightOn)
                        continue;
                    
                    float f_rad_att = 1, f_ang_att = 1;
                    
                    vec3 L = normalize(l.position - vPos);
                    vec3 V = normalize({self.attribs["viewPosition"]} - vPos);
                    
                    // infinitely light    
                    if (l.infiniteOn) {{
                        L = -1 * normalize(l.infiniteDirection);
                    }} else if (l.spotOn) {{
                        // spot light
                        vec3 vo = -1 * L;
                        L = -1 * normalize(l.spotDirection);
                        float distance = distance(vPos, l.position);
                        f_rad_att = 1.0 / dot(vec3(1, distance, distance*distance), l.spotRadialFactor);
                        float angle = dot(vo,  -1 * L);
                        if (angle >= cos(l.spotAngleLimit / 180 * 3.14159265359)) {{
                            // inside
                            f_ang_att = pow(angle, l.spotAngleFactor);
                        }} else {{
                            // outside
                            f_ang_att = 0;
                        }}
                    ...
                    if ({self.attribs["ambientOn"]}) 
                        Ia += l.color * {self.attribs["material"]}.ambient;
                    // diffuse
                    if (dot(L, N) > 0 && {self.attribs["diffuseOn"]}) {{
                        Id += {self.attribs["material"]}.diffuse * l.color * dot(N, L) * f_rad_att * f_ang_att;
                    }}
                    // specular
                    if (dot(L, N) > 0 && dot(V, R) > 0 && {self.attribs["specularOn"]}) {{
                        Is +=  {self.attribs["material"]}.specular * l.color * 
                            pow(dot(V, R), {self.attribs["material"]}.highlight) * f_rad_att * f_ang_att;
                    }}
                }}
    ...
    def setLight(self, lightIndex: int, light: Light):
        ...
        if light.spotOn:
            ...
            self.setFloat(f"""{self.attribs["light"]}[{lightIndex}].spotAngleFactor""", light.spotAngleFactor, False)
