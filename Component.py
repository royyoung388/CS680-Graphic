...
class Component:
    ...
    def draw(self, shaderProg):
        if isinstance(self.displayObj, Displayable):
            ...
            if self.normalOn:
                shaderProg.use()
                self.normal.bind(shaderProg.getUniformLocation("normalImage"))
            else:
                shaderProg.use()
                self.normal.unbind(shaderProg.getUniformLocation("normalImage"))
    ...
    def setNormal(self, shaderProg, imgFilePath, normalOn=True):
        # apply texturek
        if not os.path.isfile(imgFilePath):
            raise TypeError("Image File doesn't exist")

        shaderProg.use()
        normal_image = Image.open(imgFilePath).convert("RGB")
        normal_image = np.array(normal_image, dtype=np.uint8)
        self.normal.setTextureImage(normal_image)
