...
class Sketch(CanvasBase):
    ...
    def InitGL(self):
        ...
        self.sceneList = [SceneOne, SceneTwo, SceneThree, SceneFour, SceneFive]
        # self.sceneList = [SceneOne(self.shaderProg), SceneTwo(self.shaderProg), SceneThree(self.shaderProg)]
        self.sceneIndex = 0
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if keycode in [wx.WXK_LEFT]:
            self.sceneIndex = (self.sceneIndex - 1) % len(self.sceneList)
            self.switchScene(self.sceneList[self.sceneIndex](self.shaderProg))
            self.update()
        ...
        if chr(keycode) in "123456789":
            self.scene.toggle_light(int(chr(keycode)))
