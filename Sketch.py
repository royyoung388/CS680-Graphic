    ...
    * buff_last: Buff. Last frame buffer
    ...
    def Interrupt_MouseL(self, x, y):
        ...
        if len(self.points_l) % 2 == 1:
            ...
            self.drawLine(self.buff, self.points_l[0], self.points_l[1], self.doSmooth, self.doAA, self.doAAlevel)
            self.points_l.clear()
    ...
    def Interrupt_MouseR(self, x, y):
        ...
        if len(self.points_r) % 3 == 1:
            ...
            self.drawTriangle(self.buff, self.points_r[0], self.points_r[1], self.points_r[2], self.doSmooth, self.doAA,
                              self.doAAlevel, self.doTexture)
            self.points_r.clear()
    ...
    def drawLine(self, buff, p1, p2, doSmooth=True, doAA=False, doAAlevel=4):
        ...
        flat_color = None if doSmooth else p1.getColor()
        aa_gen = None
        if doAA:
            aa_gen = self.aa_generator(*p1.getCoords(), *p2.getCoords(), doAAlevel)
        for p in self.point_generator(p1, p2, flat_color):
            if doAA:
                res = collections.defaultdict(int)
                for _ in range(doAAlevel):
                    x, y = next(aa_gen)
                    res[(x // doAAlevel, y // doAAlevel)] += 1
                for key, value in res.items():
                    r, g, b = p.getColor().getRGB()
                    percent = value / doAAlevel
                    self.drawPoint(buff, Point(key, ColorType(r * percent, g * percent, b * percent)))
            else:
                self.drawPoint(buff, p)
    ...
    def drawTriangle(self, buff, p1, p2, p3, doSmooth=True, doAA=False, doAAlevel=4, doTexture=False):
        ...
        flat_color = None if doSmooth else p1.getColor()
        # reorder points: y1 > y2 > y3
        if p1.getCoords()[1] < p2.getCoords()[1]:
            p1, p2 = p2, p1
        if p1.getCoords()[1] < p3.getCoords()[1]:
            p1, p3 = p3, p1
        if p2.getCoords()[1] < p3.getCoords()[1]:
            p2, p3 = p3, p2

        # set texture coords
        factor = 1
        max_x = max(p1.getCoords()[0], p2.getCoords()[0], p3.getCoords()[0])
        min_x = min(p1.getCoords()[0], p2.getCoords()[0], p3.getCoords()[0])
        max_y = p1.getCoords()[1]
        min_y = p3.getCoords()[1]

        if doTexture:
            diff_x = max_x - min_x - self.texture.width + 1
            diff_y = max_y - min_y - self.texture.height + 1
            # larger than texture size
            if diff_x > 0 or diff_y > 0:
                # scale
                if diff_x > diff_y:
                    factor = (self.texture.width - 1) / (max_x - min_x)
                else:
                    factor = (self.texture.height - 1) / (max_y - min_y)

        # set texture coords offset
        for p in (p1, p2, p3):
            p.setTextureCoords((min_x, min_y))

        # render triangle
        gen12 = self.point_generator(p1, p2, flat_color)
        gen13 = self.point_generator(p1, p3, flat_color)
        gen23 = self.point_generator(p2, p3, flat_color)

        aa_gen12 = aa_gen13 = aa_gen23 = None
        # aa
        if doAA:
            aa_gen12 = self.aa_generator(*p1.getCoords(), *p2.getCoords(), doAAlevel)
            aa_gen13 = self.aa_generator(*p1.getCoords(), *p3.getCoords(), doAAlevel)
            aa_gen23 = self.aa_generator(*p2.getCoords(), *p3.getCoords(), doAAlevel)

        y1, y2, y3 = p1.getCoords()[1], p2.getCoords()[1], p3.getCoords()[1]
        y = y1
        temp12 = temp13 = p1
        temp23 = p2

        # draw triangle border.
        def draw_border(gen, temp: Point, aa_gen=None):
            nonlocal y
            # make sure y is consistent
            while temp.getCoords()[1] > y:
                temp = next(gen)
                # change texture coord and color
                if doTexture:
                    (ttx, tty) = temp.getTextureCoords()
                    (tx, ty) = temp.getCoords()
                    ttx, tty = (tx - ttx) * factor, (ty - tty) * factor
                    temp.setTextureCoords((ttx, tty))
                    temp.setColor(self.texture_bilinear(ttx, tty))
                if doAA:
                    res = collections.defaultdict(int)
                    for _ in range(doAAlevel):
                        ax, ay = next(aa_gen)
                        res[(ax // doAAlevel, ay // doAAlevel)] += 1
                    for key, value in res.items():
                        background = buff.getPixel(*key) / 255
                        r, g, b = temp.getColor().getRGB()
                        percent = value / doAAlevel
                        c = ColorType(r * percent + background[0] * (1 - percent),
                                      g * percent + background[1] * (1 - percent),
                                      b * percent + background[2] * (1 - percent))
                        self.drawPoint(buff, Point(key, c))
                else:
                    self.drawPoint(buff, temp)
            return temp

        # draw scan line
        def draw_scan_line(gen):
            for p in gen:
                if doTexture:
                    (tx, ty) = p.getTextureCoords()
                    (x, y) = p.getCoords()
                    tx, ty = (x - tx) * factor, (y - ty) * factor
                    p.setTextureCoords((tx, ty))
                    p.setColor(self.texture_bilinear(tx, ty))
                self.drawPoint(buff, p)
        ...
        while y > y2:
            ...
            temp12 = draw_border(gen12, temp12, aa_gen12)
            temp13 = draw_border(gen13, temp13, aa_gen13)

            # line fill
            temp12.setTextureCoords((min_x, min_y))
            temp13.setTextureCoords((min_x, min_y))
            draw_scan_line(self.point_generator(temp12, temp13, flat_color))
        # lower triangle
        while y > y3:
            y -= 1
            temp23 = draw_border(gen23, temp23, aa_gen23)
            temp13 = draw_border(gen13, temp13, aa_gen13)

            # line fill
            temp23.setTextureCoords((min_x, min_y))
            temp13.setTextureCoords((min_x, min_y))
            draw_scan_line(self.point_generator(temp23, temp13, flat_color))
    ...
    def texture_bilinear(self, x, y):
        ...
        #  . . .  pa (x,y) pb
        #  .   .  p3, p4
        p1 = self.queryTextureBuffPoint(self.texture, math.floor(x), ceil_y)
        p2 = self.queryTextureBuffPoint(self.texture, ceil_x, ceil_y)
        p3 = self.queryTextureBuffPoint(self.texture, math.floor(x), math.floor(y))
        p4 = self.queryTextureBuffPoint(self.texture, ceil_x, math.floor(y))
        ...
        if y == ceil_y:
            ceil_y += 1
        # |p1-pa| : |pa-p3| = a : b
        a, b = ceil_y - y, y - math.floor(y)
        # |pa-x| : |x-pb| = c : d
        c, d = x - math.floor(x), ceil_x - x

        r = p3.getColor().getRGB()[0] * a * d + p1.getColor().getRGB()[0] * d * b + p4.getColor().getRGB()[0] * c * a + \
            p2.getColor().getRGB()[0] * c * b
        g = p3.getColor().getRGB()[1] * a * d + p1.getColor().getRGB()[1] * d * b + p4.getColor().getRGB()[1] * c * a + \
            p2.getColor().getRGB()[1] * c * b
        b = p3.getColor().getRGB()[2] * a * d + p1.getColor().getRGB()[2] * d * b + p4.getColor().getRGB()[2] * c * a + \
            p2.getColor().getRGB()[2] * c * b
    ...
    # aa generator. Scale x, y according to aa level.
    def aa_generator(self, x1, y1, x2, y2, doAAlevel):
        ...
        if y2 - y1 > 0:
            ...
            y1 = (y1 + 1) * doAAlevel - 1
            y2 = y2 * doAAlevel
        return self.bresenham(x1, y1, x2, y2)

    # bresenham point generator.
    def point_generator(self, p1: Point, p2: Point, flat_Color: ColorType = None):
        (x1, y1), (x2, y2) = p1.getCoords(), p2.getCoords()
        dx = abs(x1 - x2)
        dy = abs(y1 - y2)
        step_r, step_g, step_b = 0, 0, 0

        if flat_Color is None:
            (r1, g1, b1), (r2, g2, b2) = p1.getColor().getRGB(), p2.getColor().getRGB()
            # delta of color
            dr, dg, db = r2 - r1, g2 - g1, b2 - b1
            # color change step
            if dx > dy and dx != 0:
                step_r, step_g, step_b = dr / dx, dg / dx, db / dx
            elif dy != 0:
                step_r, step_g, step_b = dr / dy, dg / dy, db / dy
        else:
            r1, g1, b1 = flat_Color.getRGB()

        for x, y in self.bresenham(*p1.getCoords(), *p2.getCoords()):
            yield Point([x, y], ColorType(r1, g1, b1), p1.getTextureCoords())
            r1, g1, b1 = r1 + step_r, g1 + step_g, b1 + step_b
